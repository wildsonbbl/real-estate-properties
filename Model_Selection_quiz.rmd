...
lmmodel<- lm(ames_train,formula=log(price)~log(Lot.Area)+log(area)+Bedroom.AbvGr+Overall.Qual+Land.Slope)

aic<-stepAIC(lmmodel,trace = F)

bic<-stepAIC(lmmodel,trace=F,k=log(1000))
aic$coefficients
bic$coefficients
```
...
model2 <- lm(data=ames_train, log(price)~log(area))
ames_train %>% 
        mutate(res = resid(model2)) %>% 
        group_by(Neighborhood) %>% 
        summarise(mean.res=mean(res)) %>% 
        arrange(desc(mean.res)) %>% head
```
...
ames_train %>% 
        mutate(res = resid(model2)) %>% 
        group_by(Neighborhood) %>% 
        summarise(mean.squared.res=mean(res^2)) %>% 
        arrange(desc(mean.squared.res)) %>% head
```
...
ames_train%>%lm(log(price)~Overall.Qual+Bsmt.Qual+Garage.Qual,data=.) -> model3
1000-length(model3$fitted.values)
```
...
ames_train %>% lm(formula=log(price)~Overall.Qual+Overall.Cond) -> model4

ames_train %>% 
        mutate(pred = model4$fitted.values) %>% 
        group_by(MS.SubClass) %>%
        summarise(media.pred = median(pred)) %>%
        arrange(desc(media.pred)) %>% head
...
data.frame(leverages=hatvalues(model4)) %>% 
        arrange(desc(leverages)) %>% head
```
...
'Holding constant all other variables in the model, on average, an additional bedroom will increase housing price by k percent.'
```
...
ames_train %>% filter(Sale.Condition == 'Normal') %>% nrow/1000*100
```
...
model5 <-  ames_train %>% filter(Sale.Condition == 'Normal') %>% lm(data=., log(price)~log(area))
