...
pred.aic.test<-exp(predict(model.AIC, ames_test))
mean(pred.aic.test)
```
...
# Extract Residuals
resid.aic.test <- ames_test$price - pred.aic.test
resid.aic.train <- ames_train$price-exp(model.AIC$fitted.values)
resid.full.train <- ames_train$price-exp(model.full$fitted.values)
...
'When predicting to ames_train, the RMSE for  model.AIC is higher than the RMSE for model.full. However, when predicting to ames_test, the RMSE for  model.full is higher.'
```
...
model.BIC <- stepAIC(model.full,k=log(nrow(ames_train)),trace =0)
...
1-pred.test.MPM.coverage %>% round(3)
```
