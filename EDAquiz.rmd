...
ames_train %>% select(Garage.Type) %>% transmute(garage = !is.na(Garage.Type)) %>% table()
```
...
ames_train %>% select(price, Street, Bsmt.Qual) %>% summary

quantile(x=ames_train$Year.Built, probs = c(0,.5,.7,1))
```
...
garage <- ames_train %>% filter(!is.na(Garage.Type)) %>% select(area) 
nogarage <- ames_train %>% filter(is.na(Garage.Type)) %>% select(area)

t.test(x = garage, y = nogarage,var.equal =T)
```
...
bedrooms <- ames_train %>% filter(area>2000) %>% select(Bedroom.AbvGr)
...
k=(mubefore/sigmabefore)^2
theta = mubefore/k

kposterior = k + sum(bedrooms$Bedroom.AbvGr)
thetaposterior = theta/(theta*n+1)

mu<-kposterior*thetaposterior
sigma <- thetaposterior*sqrt(kposterior)
round(mu,2)
round(sigma,2)
```
...
linearmodel<- lm(log(price)~log(area),ames_train)
res<-data.frame(res = linearmodel$residuals^2) %>% arrange(desc(res)) %>% head(3)
linhas <- as.numeric(rownames(res))

ames_train[linhas,] %>% select(price,area,Year.Built,Sale.Condition,Overall.Qual,Bedroom.AbvGr)
```
...
ames_train %>% group_by(Neighborhood) %>% summarise(prop = mean(Bldg.Type == "1Fam")) %>% filter(prop == 1)

table(ames_train$Neighborhood,ames_train$Bldg.Type)
```

```{r}
g3<-ggplot(ames_train, aes(x = Neighborhood))+ theme_bw()
g3<-g3+geom_bar(aes(fill = Bldg.Type)) 
g3<-g3+geom_text(stat='count',aes(label = Neighborhood),position = position_stack(vjust=.5),angle = 90,size=2.5)  
g3<-g3+scale_x_discrete(label = NULL) 
g3
```
...
ggplot(ames_train, aes(x = Bedroom.AbvGr, y = log(area))) + geom_point() + geom_smooth(method = 'lm') + theme_bw()
```
